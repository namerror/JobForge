...
def generate_education_id(name: str, existing_ids: set[str]) -> str:
    return generate_record_id(name, existing_ids, "education")
...
class EducationEvidenceSession:
    ...
    def create_education(
        ...
        *,
        ...
        existing_ids = {education["id"] for education in staged_data["education"]}
        education_data = {
            "id": generate_education_id(name, existing_ids),
            "name": name,
            "degree": degree,
            "grade": grade,
            "start": start,
    ...
    def update_education(
        ...
        *,
        ...
        staged_data = _education_file_to_data(self._staged)
        resolved_index = self._resolve_index(index)
        original_education = staged_data["education"][resolved_index]
        staged_data["education"][resolved_index] = {
            "id": original_education["id"],
            "name": name,
            "degree": degree,
            "grade": grade,
            "start": start,
    ...
    def pending_changes(self) -> PendingEducationChanges:
        baseline_education = {item.id: item for item in self._baseline.education}
        staged_education = {item.id: item for item in self._staged.education}

        created = tuple(
            staged_education[education_id].name
            for education_id in staged_education
            if education_id not in baseline_education
        )
        deleted = tuple(
            baseline_education[education_id].name
            for education_id in baseline_education
            if education_id not in staged_education
        )
        updated = tuple(
            staged_education[education_id].name
            for education_id in staged_education
            if education_id in baseline_education
            and staged_education[education_id].model_dump(mode="python")
            != baseline_education[education_id].model_dump(mode="python")
        )
